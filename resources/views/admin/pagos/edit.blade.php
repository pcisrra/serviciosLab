    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pagos.update", [$pago->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuario_cobra') ? 'is-invalid' : '' }}" name="usuario_cobra_id" id="usuario_cobra_id" required>
                    @foreach($usuario_cobras as $id => $usuario_cobra)
                        <option value="{{ $id }}" {{ ($pago->usuario_cobra ? $pago->usuario_cobra->id : old('usuario_cobra_id')) == $id ? 'selected' : '' }}>{{ $usuario_cobra }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('beneficiario_pago') ? 'is-invalid' : '' }}" name="beneficiario_pago_id" id="beneficiario_pago_id" required>
                    @foreach($beneficiario_pagos as $id => $beneficiario_pago)
                        <option value="{{ $id }}" {{ ($pago->beneficiario_pago ? $pago->beneficiario_pago->id : old('beneficiario_pago_id')) == $id ? 'selected' : '' }}>{{ $beneficiario_pago }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_pago') ? 'is-invalid' : '' }}" type="text" name="fecha_pago" id="fecha_pago" value="{{ old('fecha_pago', $pago->fecha_pago) }}" required>
