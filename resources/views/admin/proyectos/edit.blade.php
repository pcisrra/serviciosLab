    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.proyectos.update", [$proyecto->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('codigo_proyecto') ? 'is-invalid' : '' }}" type="text" name="codigo_proyecto" id="codigo_proyecto" value="{{ old('codigo_proyecto', $proyecto->codigo_proyecto) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $cliente)
                        <option value="{{ $id }}" {{ ($proyecto->cliente ? $proyecto->cliente->id : old('cliente_id')) == $id ? 'selected' : '' }}>{{ $cliente }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('maquina_asignada') ? 'is-invalid' : '' }}" name="maquina_asignada_id" id="maquina_asignada_id" required>
                    @foreach($maquina_asignadas as $id => $maquina_asignada)
                        <option value="{{ $id }}" {{ ($proyecto->maquina_asignada ? $proyecto->maquina_asignada->id : old('maquina_asignada_id')) == $id ? 'selected' : '' }}>{{ $maquina_asignada }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_propuesta') ? 'is-invalid' : '' }}" type="text" name="fecha_propuesta" id="fecha_propuesta" value="{{ old('fecha_propuesta', $proyecto->fecha_propuesta) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuario_acepta') ? 'is-invalid' : '' }}" name="usuario_acepta_id" id="usuario_acepta_id" required>
                    @foreach($usuario_aceptas as $id => $usuario_acepta)
                        <option value="{{ $id }}" {{ ($proyecto->usuario_acepta ? $proyecto->usuario_acepta->id : old('usuario_acepta_id')) == $id ? 'selected' : '' }}>{{ $usuario_acepta }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuario_cargo') ? 'is-invalid' : '' }}" name="usuario_cargo_id" id="usuario_cargo_id" required>
                    @foreach($usuario_cargos as $id => $usuario_cargo)
                        <option value="{{ $id }}" {{ ($proyecto->usuario_cargo ? $proyecto->usuario_cargo->id : old('usuario_cargo_id')) == $id ? 'selected' : '' }}>{{ $usuario_cargo }}</option>
                    @endforeach
                </select>
