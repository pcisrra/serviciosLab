...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.maquinas.update", [$maquina->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="codigo">{{ trans('cruds.maquina.fields.codigo') }}</label>
                <input class="form-control {{ $errors->has('codigo') ? 'is-invalid' : '' }}" type="text" name="codigo" id="codigo" value="{{ old('codigo', $maquina->codigo) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $maquina->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" type="text" name="estado" id="estado" value="{{ old('estado', $maquina->estado) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('disponibilidad') ? 'is-invalid' : '' }}" name="disponibilidad" id="disponibilidad">
                    ...
                    @foreach(App\Maquina::DISPONIBILIDAD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('disponibilidad', $maquina->disponibilidad) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
